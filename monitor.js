...
function hy2Api(path) {
  return new Promise(resolve => {
    const req = http.request({ hostname: '127.0.0.1', port: 8388, path }, res => {
      let d = '';
      res.on('data', c => d += c);
...
function getConnections() {
  try {
    const logOut = cmd("journalctl -u hysteria-server --since '10 minutes ago' --no-pager -q 2>/dev/null");
    if (!logOut) return { count: 0, ips: [], recentIps: [] };
    ...
    const ips = [...new Set(connected.values())];
    return { count: ips.length, ips, recentIps: [...recentIps] };
  } catch(e) { return { count: 0, ips: [], recentIps: [] }; }
}
...
async function collect() {
  try {
    ...
    const hy2 = await hy2Api('/traffic');
    let hrx = 0, htx = 0;
...
router.get('/api', async (req, res) => {
  ...
  const up = os.uptime();
  const tm = os.totalmem(), fm = os.freemem();
  const [hy2Traffic, hy2Online] = await Promise.all([hy2Api('/traffic'), hy2Api('/online')]);

  let proxyRx = 0, proxyTx = 0;
  if (hy2Traffic && hy2Traffic.user) { proxyRx = hy2Traffic.user.rx || 0; proxyTx = hy2Traffic.user.tx || 0; }

  let connCount = 0;
  if (hy2Online && typeof hy2Online === 'object') {
    connCount = Object.values(hy2Online).reduce((s, v) => s + (typeof v === 'number' ? v : 0), 0);
  }

  const { ips: connIPs, recentIps } = getConnections();
  if (connIPs.length > connCount) connCount = connIPs.length;
  ...
  res.json({
    ...
    traffic,
    proxyRx: fmtTraffic(proxyRx),
    proxyTx: fmtTraffic(proxyTx),
    connCount,
    connIPs,
    recentIps,
...
async function loadCurrent() {
  try {
    ...
    var todayTraffic = '<div class="row"><span class="lbl">数据收集中...</span></div>';
    if (d.traffic) {
      todayTraffic =
        '<div class="row"><span class="lbl">↓ 接收</span><span class="val">' + d.traffic.rx + '</span></div>' +
        '<div class="row"><span class="lbl">↑ 发送</span><span class="val">' + d.traffic.tx + '</span></div>';
    }

    var ipList = '';
    if (d.connIPs && d.connIPs.length > 0) {
      ipList = d.connIPs.map(function(ip) {
        return '<div class="ip-tag online"><span class="dot green-dot"></span>' + ip + '</div>';
      }).join('');
    } else if (d.recentIps && d.recentIps.length > 0) {
      ipList = d.recentIps.map(function(ip) {
        return '<div class="ip-tag recent"><span class="dot grey-dot"></span>' + ip + ' <span class="recent-lbl">10分钟内</span></div>';
      }).join('');
    } else {
      ipList = '<span class="lbl">暂无连接</span>';
    }
    ...
    document.getElementById('g').innerHTML =
      ...
      '</div>' +
      '<div class="card"><div class="ct">实时网速 <span class="hint">含所有流量</span></div>' +
        '<div class="speeds">' +
      ...
      '</div>' +
      '<div class="card"><div class="ct">今日系统流量 <span class="hint">vnstat</span></div>' + todayTraffic + '</div>' +
      '<div class="card"><div class="ct">代理累计流量 <span class="hint">Hysteria2 本次启动</span></div>' +
        '<div class="row"><span class="lbl">↓ 客户端下载</span><span class="val green">' + d.proxyTx + '</span></div>' +
        '<div class="row"><span class="lbl">↑ 客户端上传</span><span class="val pink">' + d.proxyRx + '</span></div>' +
      '</div>' +
      '<div class="card"><div class="ct">当前连接 <span class="badge-count">' + d.connCount + '</span></div>' +
...
async function loadHistory() {
  try {
    ...
    var chartOpts = { responsive: true, maintainAspectRatio: false, animation: false,
      ...
      scales: { x: { ticks: { color: '#475569', maxTicksLimit: 8 }, grid: { color: '#0f172a' } },
                y: { ticks: { color: '#475569' }, grid: { color: '#0f172a' }, min: 0 } } };
    ...
    if (trafficChart) {
      ...
      trafficChart = new Chart(document.getElementById('trafficChart'), {
        ...
        options: { responsive: true, maintainAspectRatio: false, animation: false,
          plugins: { legend: { labels: { color: '#94a3b8', font: { size: 11 }, boxWidth: 12 } } },
          scales: { x: { ticks: { color: '#475569' }, grid: { color: '#0f172a' } },
                    y: { ticks: { color: '#475569' }, grid: { color: '#0f172a' }, min: 0 } } }
      });
    }
...
.ct{font-size:11px;color:#64748b;text-transform:uppercase;letter-spacing:1px;margin-bottom:14px;display:flex;align-items:center;gap:8px}
.hint{font-size:10px;color:#475569;text-transform:none;letter-spacing:0;background:#0f172a;padding:2px 6px;border-radius:4px}
...
.badge-count{background:#1d4ed8;color:#93c5fd;padding:2px 8px;border-radius:20px;font-size:12px;font-weight:700}
...
.ip-list{display:flex;flex-wrap:wrap;gap:6px;margin-top:4px}
.ip-tag{background:#0f172a;border:1px solid #334155;color:#94a3b8;font-size:12px;padding:3px 10px;border-radius:6px;font-family:monospace;display:flex;align-items:center;gap:5px}
.ip-tag.online{border-color:#166534;color:#4ade80}
.ip-tag.recent{border-color:#334155;color:#64748b}
.dot{width:7px;height:7px;border-radius:50%;display:inline-block;flex-shrink:0}
.green-dot{background:#4ade80;box-shadow:0 0 6px #4ade80}
.grey-dot{background:#475569}
.recent-lbl{font-size:10px;color:#475569;margin-left:2px}
.section{max-width:1100px;margin:24px auto 0}
